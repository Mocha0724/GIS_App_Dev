...
namespace MapObjects
{
    public class ProjectionCS
    {
        ...
        //投影参数
        private ProjectionTypeConstant _ProjType; //投影类型
        ...
        private LinearUnitConstant _LinearUnit;   //投影后的长度单位
        ...
        public ProjectionCS(String projCSName, String geoCSName, String datumName, string spheroidName, double semiMajor, double inverseFlattening,
            ProjectionTypeConstant projType, double originLatitude, double centralMeridian, double falseEasting, double falseNorthing, double scaleFactor,
                double standardParallelOne, double standardParallelTwo, LinearUnitConstant linearUnit)
        {
        ...
        //获取投影类型
        public ProjectionTypeConstant ProjType
        {
        ...
        //获取投影后的长度单位
        public LinearUnitConstant LinearUnit
        {
        ...
        /// <returns></returns>
        public GeoPoint TransferToProjCo(GeoPoint LngLat)
        {
            GeoPoint sResult;
            ...
            if (_ProjType == ProjectionTypeConstant.None)
                sResult = new GeoPoint(LngLat.X, LngLat.Y);
            else if (_ProjType == ProjectionTypeConstant.Mercator)
                throw new NotImplementedException();
            else if (_ProjType == ProjectionTypeConstant.UTM)
                throw new NotImplementedException();
            else if (_ProjType == ProjectionTypeConstant.Gauss_Kruger)
                throw new NotImplementedException();
            else if (_ProjType == ProjectionTypeConstant.Lambert_Conformal_Conic_2SP)
                sResult = LambertFwd(LngLat);
            else if (_ProjType == ProjectionTypeConstant.Albers_Equal_Area)
                throw new NotImplementedException();
            else
                throw new NotImplementedException();
            return sResult;
        ...
        /// <returns></returns>
        public GeoPoint TransferToLngLat(GeoPoint ProjCo)
        {
            GeoPoint sResult;
            if (_ProjType == ProjectionTypeConstant.None)
                sResult = new GeoPoint(ProjCo.X, ProjCo.Y);
            else if (_ProjType == ProjectionTypeConstant.Mercator)
                throw new NotImplementedException();
            else if (_ProjType == ProjectionTypeConstant.UTM)
                throw new NotImplementedException();
        ...
        {
            double sDataOut = 0;
            if (_LinearUnit == LinearUnitConstant.Millimeter)
                sDataOut = dataIn * 1000;
            else if (_LinearUnit == LinearUnitConstant.Centimeter)
                sDataOut = dataIn * 100;
            else if (_LinearUnit == LinearUnitConstant.Decimeter)
                sDataOut = dataIn * 10;
            else if (_LinearUnit == LinearUnitConstant.Meter)
                sDataOut = dataIn;
            else if (_LinearUnit == LinearUnitConstant.Kilometers)
        ...
        private void InitializeVars()
        {
            ...
            ep2 = ep * ep;
            //根据投影类型设置特殊变量的值
            if (_ProjType == ProjectionTypeConstant.Mercator)
            { throw new NotImplementedException(); }
            else if (_ProjType == ProjectionTypeConstant.Gauss_Kruger)
            { throw new NotImplementedException(); }
            else if (_ProjType == ProjectionTypeConstant.UTM)
        ...
        //将经纬度转换为投影坐标
        private GeoPoint LambertFwd(GeoPoint LngLat)
        {
            ...
            GeoPoint sResult = new GeoPoint(x, y);
            return sResult;
        }

        //将投影坐标转换为经纬度
        private GeoPoint LambertInv(GeoPoint ProjCo)
        {
            ...
            if ((r != 0) || (n > 0.0))
            {
                ...
                lat = -HALF_PI;
            lng = theta / n + lambda0;
            GeoPoint sResult = new GeoPoint(ToDegrees(lng), ToDegrees(lat));
            return sResult;
        }
